...
<head>
    ...
    <script>
        window.listeLivreurs = <?= json_encode(array_map(fn($l) => [
            'email'  => $l['email'],
            'nom'    => $l['nom'],
            'prenom' => $l['prenom']
        ], $livreurs)) ?>;
...
<div class="att" style="padding-top:2rem;">
    ...
    <form method="GET" action="restaurateur.php">
        <select name="statut" class="selection">
            ...
            <option value="En attente de paiement" <?= ($_GET['statut'] ?? '') === 'En attente de paiement' ? 'selected' : '' ?>>En attente de paiement</option>
                            ...
                            <?php elseif ($statutCmd === 'En préparation'): ?>
                                <div class="zone-assignation">
                                    ...
                                    <button class="btn-statut btn-prete"
                                        onclick="gererStatut(this, <?= $idCmd ?>, 'prete')">
                                        Prête & Assigner
                                    </button>
                                </div>
                            <?php else: ?>
